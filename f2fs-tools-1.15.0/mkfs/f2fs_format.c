...
static int f2fs_prepare_super_block(void)
{
  ...
  uint32_t zone_size_bytes;
	...
	int i;
#if META_FOR_ZNS
	uint32_t blkz_cap_blks, blkz_size_blks, blkz_size_segs;
	uint32_t zone_needed;
	blkz_cap_blks = c.devices[0].zone_cap_blocks[0];
	blkz_size_blks = c.devices[0].zone_blocks;
...
#else
  zone_size_bytes =
        ((uint64_t) blk_size_bytes * c.secs_per_zone *
  ...
  MSG(1, "Debug: total available blocks : %u, blocks_for_ssa : %u, segment_count_ssa : %u\n",
    total_valid_blks_available, blocks_for_ssa, get_sb(segment_count_ssa));
...
// adjust SSA segment count to align to blkzone
  ...
  if(diff) {
	...
	if ((!(c.feature & cpu_to_le32(F2FS_FEATURE_RO)) &&
		...
		MSG(0, "\tError: op : %f, rsv_segs: %u\n", c.overprovision, c.reserved_segments);
		MSG(0, "\tError: segs: %u, : %u\n", f2fs_get_usable_segments(sb), get_sb(section_count));
		return -1;
	}
	if (c.vol_uuid) {
