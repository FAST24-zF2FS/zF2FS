...
#if 0	
		printk("(%s : %d) old(%u) inuse:%u, new(%u) inuse:%u",
			__func__, __LINE__, 
			old_segno,
			is_inuse_seg(sbi, old_segno),
			segno,
			is_inuse_seg(sbi, segno));
		
		printk("(%s : %d) old(%u) sec:%u, new(%u) sec:%u",
			__func__, __LINE__, 
			old_segno,
...
static int merge_sit_set(struct f2fs_sb_info *sbi, struct sit_entry_set *set, int foreground){
  ...
  if (set->entry_cnt) {	
    printk("(%s : %d) error segno(%u), cnt(%u)",
...
#else
      if (i == CURSEG_WARM_DATA || i == CURSEG_COLD_DATA) {
			  array[i].allocated_segs = f2fs_kzalloc(sbi, 
					SM_I(sbi)->stripe_cnt * sizeof(unsigned int), GFP_KERNEL);
      } else {
        array[i].allocated_segs = f2fs_kzalloc(sbi, 
					SM_I(sbi)->stripe_min_cnt * sizeof(unsigned int), GFP_KERNEL);
      }
#endif
		}
#endif
